...
def model_cnnlstm(modelname_tosave):
    ...
    plt.tight_layout()
    plt.savefig('results/model_cnnlstm_bs32.png')
    plt.close()
    score = model.evaluate(Xtest, Ytest, verbose=1)
    print("Test score cnnlstm: {:.3f}, accuracy: {:.3f}".format(score[0], score[1]))
    stop = timeit.default_timer()
    train_time=stop - start
    with open("results/cnnlstm_result.txt", "a") as myfile:
        myfile.write("(cnnlstm_model(1Layer,bs32,hl128,em100,nfilter=64,filter=3) \n accuracy={0}\n validation_accuracy={1}\n loss={2}\n validation_loss={3}\n test_score={4}, train_time={5}\n".format(
        accuracy_rslt,val_accuracy_rslt,loss_rslt,val_loss_rslt,score[1],train_time))
        
def model_cnnlstm_pre(modelname_tosave):
    start = timeit.default_timer()
    model = Sequential()
    # CONV => RELU => POOL
    ...
    plt.plot(history.history["val_loss"], color="g", label="Validation")
    plt.tight_layout()
    plt.savefig('results/model_cnnlstm_pre_bs32.png')
    plt.close()
    score = model.evaluate(Xtest, Ytest, verbose=1)
    print("Test score cnnlstm: {:.3f}, accuracy: {:.3f}".format(score[0], score[1]))
    stop = timeit.default_timer()
    train_time=stop - start
    with open("results/cnnlstm_result.txt", "a") as myfile:
        myfile.write("(cnnlstm_model_pre(1Layer,bs32,hl128,em100,nfilter64,filter3) \n accuracy={0}\n validation_accuracy={1}\n loss={2}\n validation_loss={3}\n test_score={4}, train_time={5}\n".format(
        accuracy_rslt,val_accuracy_rslt,loss_rslt,val_loss_rslt,score[1],train_time))

model_cnnlstm("modelcnnlstm_bs32")
model_cnnlstm_pre("modelcnnlstm_pre_bs32")